...
export const asciiLower = (s: string): string => {
	...
	for (let i = 0; i < s.length; i++) {
		const ch = s[i];
		out += ch !== undefined && ch >= "A" && ch <= "Z" ? String.fromCharCode(ch.charCodeAt(0) + 32) : ch;
	}
	return out;
};
...
export function stripMarkdown(s: string): string {
	...
	const noStrong = noCode.split("**").join("").split("__").join("");
	...
	for (let i = 0; i < chars.length; i++) {
		...
		const inWord =
			i > 0 && isAlphanumeric(chars[i - 1]) && chars[i + 1] !== undefined && isAlphanumeric(chars[i + 1]);
