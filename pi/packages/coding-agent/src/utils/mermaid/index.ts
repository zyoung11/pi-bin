...
export function render(src: string): MermaidArt | null {
	...
	const canvasLines = drawn.canvas.toLines();
	return { ...canvasLines, warnings: drawn.warnings };
}
...
function draw(src: string): Drawn | null {
	const plain = (canvas: CanvasResult): Drawn | null =>
		canvas === null ? null : { canvas, warnings: [] as string[] };
