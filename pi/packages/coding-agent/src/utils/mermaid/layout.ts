/** scriptc port: new Array/fill/Array.from have no lowering — build by loop. */
function filledArray<T>(count: number, make: (index: number) => T): T[] {
	const out: T[] = [];
...
import { Canvas, CONT, D, drawText, drawTextOverEdges, L, R, STY_DOT, STY_SOLID, STY_THICK, U } from "./canvas.ts";
...
export function computeRanks(graph: Graph): number[] {
	...
	const children: number[][] = filledArray(n, () => []);
	const indeg = filledArray(n, () => 0);
...
function dfsDag(start: number, children: number[][], color: Uint8Array, dag: number[][], order: number[]): void {
...
export function orderRanks(byRank: number[][], edges: Edge[], ranks: number[]): void {
	...
	const parents: number[][] = filledArray(n, () => []);
	const children: number[][] = filledArray(n, () => []);
	...
	for (let i = 0; i < byRank.length; i++) {
		...
		for (const v of best[i]) replacement.push(v);
		byRank[i] = replacement;
	}
}
...
export function assignPositions(
	...
	return filledArray(n, (v: number) => Math.max(0, Math.round(pos[v] - minLeft)));
}
...
function busSpans(graph: Graph, ranks: number[], centers: number[], r: number, exact: boolean): Span5[] {
	const out: Span5[] = [];
...
function placeTd(
	...
	const edgeBus = filledArray(graph.edges.length, () => 0);
	const busTracks = filledArray(maxRank + 1, () => 0);
	...
	const edgeLane = filledArray(graph.edges.length, () => 0);
...
function placeLr(
	...
	const rankX = filledArray(maxRank + 1, () => 0);
	for (let r = 1; r <= maxRank; r++) {
	...
	const bandEnd = filledArray(maxRank + 1, (r: number) => rankX[r] + colW[r]);
...
export function layoutCanvas(graph: Graph, extras: NodeExtra[]): CanvasResult {
	...
	const byRank: number[][] = filledArray(maxRank + 1, () => []);
	...
	const boxW = extras.map((extra, i) => {
		...
		if (extra.kind === "compartments") {
			let flat: string[] = [];
			for (const section of extra.sections) flat = flat.concat(section);
			return widest(flat) + 2 * PAD + 2;
		}
	...
	const extraH = filledArray(n, () => 0);
	const selfLabelW = filledArray(n, () => 0);
	for (const e of graph.edges) {
	...
	const placed: Placed[] = filledArray(n, () => ({
		x: 0,
		y: 0,
		w: 0,
		h: 0,
...
export function layoutGrouped(graph: Graph): CanvasResult {
	...
	const scopeEdges = new Map<number, [ItemKey, ItemKey, number][]>();
	const referenced = filledArray(graph.groups.length, () => false as boolean);
	...
	const directNodes = new Map<number, number[]>();
	graph.nodeGroup.forEach((g, ni) => {
		if (proxy.has(ni)) return;
		const gi = g === null ? -1 : g;
		const list = directNodes.get(gi);
		if (list) list.push(ni);
		else directNodes.set(gi, [ni]);
	});

	// Drop empty subgraphs, but keep any that an edge attaches to.
	const keep = filledArray(graph.groups.length, () => false as boolean);
	...
	const canvas = buildScope(graph, -1, scopeEdges, directNodes, keep);
	return canvas && orient(canvas, graph);
}
...
function buildScope(
	graph: Graph,
	scope: number,
	scopeEdges: Map<number, [ItemKey, ItemKey, number][]>,
	directNodes: Map<number, number[]>,
	keep: boolean[],
): CanvasResult {
	const items: ItemKey[] = (directNodes.get(scope) ?? []).map(nodeKey);
	const childGroups = graph.groups
		.map((_, gi) => gi)
		.filter((gi) => graph.groups[gi].parent === (scope === -1 ? null : scope) && keep[gi]);
