...
export function layoutSequence(seq: Sequence): CanvasResult {
	...
	for (const [l, r, need] of reqs) {
		let cur = 0;
		for (let i = l; i < r; i++) cur = cur + gaps[i];
		if (cur < need) gaps[r - 1] = gaps[r - 1] + (need - cur);
	}

	const xs: number[] = [];
	for (let i = 0; i < n; i++) xs.push(0);
	xs[0] = half(boxW[0]);
...
function drawMessage(canvas: Canvas, item: Extract<SeqItem, { kind: "message" }>, xs: number[], r: number): void {
