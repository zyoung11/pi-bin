...
/** scriptc port: array index reads past the end trap instead of returning
 * undefined; the parsers probe past the end constantly, so reads go through
 * this accessor (empty string behaves like undefined for every comparison). */
function charAt(chars: string[], index: number): string {
...
function parseNodeGroup(chars: string[], start: number, graph: Graph): { group: number[]; next: number } | null {
...
function skipSpaces(chars: string[], i: number): number {
	while (i < chars.length && (charAt(chars, i) === " " || charAt(chars, i) === "\t")) i++;
	return i;
}

function parseNode(chars: string[], start: number, graph: Graph): { index: number; next: number } | null {
	let i = skipSpaces(chars, start);
	const idStart = i;
	while (i < chars.length && isIdChar(charAt(chars, i))) i++;
...
function readShapeAt(chars: string[], i: number): Shaped {
	if (i >= chars.length) return { shape: "rect", label: null, after: i };
	const c = charAt(chars, i);
	const n = i + 1 < chars.length ? charAt(chars, i + 1) : undefined;
	if (c === "[") {
...
function readShape(chars: string[], start: number, closer: string, shape: Shape): Shaped {
	let j = start;
	while (j < chars.length && (charAt(chars, j) === " " || charAt(chars, j) === "\t")) j++;
	const quoted = j < chars.length && charAt(chars, j) === '"';
...
const isLinkChar = (c: string): boolean => c === "-" || c === "." || c === "=" || c === "<" || c === ">";
...
function parseLink(chars: string[], start: number): Link | null {
	...
	if (
		(charAt(chars, i) === "o" || charAt(chars, i) === "x") &&
		(charAt(chars, i + 1) === "-" || charAt(chars, i + 1) === "." || charAt(chars, i + 1) === "=")
	) {
		left = charAt(chars, i) === "o" ? "circle" : "cross";
		i++;
	}

	const opStart = i;
	while (i < chars.length && isLinkChar(charAt(chars, i))) i++;
	...
	if (charAt(chars, i) === "|") {
		i++;
		const lStart = i;
		while (i < chars.length && charAt(chars, i) !== "|") i++;
	...
	if (right === "none") {
		...
		let j = textStart;
		while (j < chars.length && !isLinkChar(charAt(chars, j))) j++;
		if (j < chars.length && j > textStart && charAt(chars, j) !== "<") {
...
function trailingHead(chars: string[], i: number): { head: Head; next: number } | null {
	const head: Head | null = charAt(chars, i) === "o" ? "circle" : charAt(chars, i) === "x" ? "cross" : null;
	if (head === null) return null;
	const after = charAt(chars, i + 1);
	const boundary =
		after === undefined || after === " " || after === "\t" || after === "|" || after === "&" || after === ";";
...
export function parseClass(src: string): { graph: Graph; infos: ClassInfo[] } | null {
	...
	for (const st of statements.slice(1)) {
		...
		if (["note", "callback", "click", "link", "style", "cssclass", "classdef", "namespace", "}"].includes(first)) {
			continue;
		}
		if (first === "class") {
...
function parseClassRelation(st: string): ClassRelation | null {
	...
	outer: for (let pos = 0; pos < chars.length; pos++) {
		...
		for (const [op, headFrom, headTo, line] of CLASS_OPS) {
			...
			if (op.startsWith("o") && pos > 0 && isIdChar(charAt(chars, pos - 1))) continue;
			const after = charAt(chars, pos + op.length);
