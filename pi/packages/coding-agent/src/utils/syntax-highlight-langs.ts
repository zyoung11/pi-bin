...
import { LANG_DART, LANG_JS_TEMPLATE_LITERALS, LANG_KT, LANG_MD } from "./syntax-highlight-hand.ts";
...
const LANG_ASM: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /\$[\da-fA-F]*\b/, flags: "", type: "num", sub: "", subRules: NO_SUB_RULES },
		{
			...
			type: "kwd",
			sub: "",
			subRules: [{ re: /^[a-z]+/, flags: "", type: "func", sub: "", subRules: NO_SUB_RULES }],
		},
...
const LANG_BASH: ShjLanguageDefinition = {
	rules: [
		{ re: /#.*/, flags: "", type: "", sub: "todo", subRules: NO_SUB_RULES },
		{
			...
			type: "str",
			sub: "",
			subRules: [{ re: /\$\w+|\${[^}]*}|\$\([^)]*\)/, flags: "", type: "var", sub: "", subRules: NO_SUB_RULES }],
		},
		{ re: /(?<=\s|^)\.*\/[a-z/_.-]+/i, flags: "i", type: "oper", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\s-[a-zA-Z]+|$<|[&|;]+|\b(unset|readonly|shift|export|if|fi|else|elif|while|do|done|for|until|case|esac|break|continue|exit|return|trap|wait|eval|exec|then|declare|enable|local|select|typeset|time|add|remove|install|update|delete)(?=\s|$)/,
			flags: "",
			type: "kwd",
			sub: "",
...
const LANG_C: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /(\.e?|\b)\d(e-|[\d.oxa-fA-F_])*(\.|\b)/, flags: "", type: "num", sub: "", subRules: NO_SUB_RULES },
		{
			...
			type: "kwd",
			sub: "",
			subRules: [{ re: /(<|").*/, flags: "", type: "str", sub: "", subRules: NO_SUB_RULES }],
		},
		{
			re: /asm\s*{[^}]*}/,
			flags: "",
		...
		{
			re: /\*|&|#[a-z]+\b|\b(asm|auto|double|int|struct|break|else|long|switch|case|enum|register|typedef|char|extern|return|union|const|float|short|unsigned|continue|for|signed|void|default|goto|sizeof|volatile|do|if|static|while)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_CS: ShjLanguageDefinition = {
	rules: [
		...
		{
			re: /\b0[xb][\da-f_]+[ul]*|(\b\d|\.\d)[\d_]*(\.\d[\d_]*)?(e[+-]?\d+)?[dflmu]*\b/i,
			flags: "i",
			type: "num",
			sub: "",
			subRules: NO_SUB_RULES,
		},
		{
			...
			type: "",
			sub: "",
			subRules: [{ re: /#\s*\w+/, flags: "", type: "kwd", sub: "", subRules: NO_SUB_RULES }],
		},
...
const LANG_CSS: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /(["'])(\\[^]|(?!\1)[^\r\n\\])*\1?/, flags: "", type: "str", sub: "", subRules: NO_SUB_RULES },
		{
			re: /@\w+\b|\b(and|not|only|or)\b|\b(?=([a-z-]+))\2(?=[^{}]*{)/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /\d+(\.\d+)?(cm|mm|in|px|pt|pc|em|ex|ch|rem|vm|vh|vmin|vmax|%)?/,
			flags: "",
			type: "num",
			sub: "",
			subRules: [{ re: /[a-z]+|%/, flags: "", type: "var", sub: "", subRules: NO_SUB_RULES }],
		},
		{
			re: /url\([^)]*\)/,
			flags: "",
...
const LANG_DOCKER: ShjLanguageDefinition = {
	rules: [
		{
			re: /^(FROM|RUN|CMD|LABEL|MAINTAINER|EXPOSE|ENV|ADD|COPY|ENTRYPOINT|VOLUME|USER|WORKDIR|ARG|ONBUILD|STOPSIGNAL|HEALTHCHECK|SHELL)\b/im,
			flags: "im",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_GRAPHQL: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /\b(true|false|null)\b/, flags: "", type: "bool", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b(query|mutation|subscription|fragment|on|type|input|interface|union|enum|scalar|schema|directive|extend|implements|repeatable)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_HTML: ShjLanguageDefinition = {
	rules: [
		...
		{
			re: /<style(\s+[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*\s*(=\s*([^"'>\s][^>\s]*|("|')(\\[^]|(?!\4)[^])*\4?)?)?)*\s*>[^]*?<\/style\s*>/,
			flags: "",
			type: "",
			sub: "",
			subRules: [
				{
					re: /^<style(\s+[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*\s*(=\s*([^"'>\s][^>\s]*|("|')(\\[^]|(?!\4)[^])*\4?)?)?)*\s*>/,
					flags: "",
					type: "",
					sub: "",
					subRules: [
				...
				{
					re: /\/<[/!?]?[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*(\s+[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*\s*(=\s*([^"'>\s][^>\s]*|("|')(\\[^]|(?!\4)[^])*\4?)?)?)*\s*[/!?]?>\/g|[^]*(?=<\/style\s*>$)/,
					flags: "",
					type: "",
					sub: "css",
					subRules: NO_SUB_RULES,
				...
				{
					re: /<[/!?]?[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*(\s+[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*\s*(=\s*([^"'>\s][^>\s]*|("|')(\\[^]|(?!\4)[^])*\4?)?)?)*\s*[/!?]?>/,
					flags: "",
					type: "",
					sub: "",
					subRules: [
		...
		{
			re: /<script(\s+[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*\s*(=\s*([^"'>\s][^>\s]*|("|')(\\[^]|(?!\4)[^])*\4?)?)?)*\s*>[^]*?<\/script\s*>/,
			flags: "",
			type: "",
			sub: "",
			subRules: [
				{
					re: /^<script(\s+[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*\s*(=\s*([^"'>\s][^>\s]*|("|')(\\[^]|(?!\4)[^])*\4?)?)?)*\s*>/,
					flags: "",
					type: "",
					sub: "",
					subRules: [
				...
				{
					re: /\/<[/!?]?[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*(\s+[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*\s*(=\s*([^"'>\s][^>\s]*|("|')(\\[^]|(?!\4)[^])*\4?)?)?)*\s*[/!?]?>\/g|[^]*(?=<\/script\s*>$)/,
					flags: "",
					type: "",
					sub: "js",
					subRules: NO_SUB_RULES,
...
const LANG_HTTP: ShjLanguageDefinition = {
	rules: [
		{
			re: /^(GET|HEAD|POST|PUT|DELETE|CONNECT|OPTIONS|TRACE|PATCH|PRI|SEARCH)\b/m,
			flags: "m",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_JS: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /\b[A-Z][\w_]*\b/, flags: "", type: "class", sub: "", subRules: NO_SUB_RULES },
		{
			re: /[a-zA-Z$_][\w$_]*(?=\s*((\?\.)?\s*\(|=\s*(\(?[\w,{}[\])]+\)? =>|function\b)))/,
			flags: "",
			type: "func",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_JSDOC: ShjLanguageDefinition = {
	rules: [
		...
		{
			re: /\b(TODO|FIXME|DEBUG|OPTIMIZE|WARNING|XXX|BUG)\b/,
			flags: "",
			type: "err",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_JSX: ShjLanguageDefinition = {
	rules: [
		{
			re: /(?<![$\w])<([:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*)(?:\s+[$\w{}.:-]+(?:\s*=\s*(?:\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}|"[^"]*"?|'[^']*'?|[^"'>\s][^>\s]*))?)*\s*>[^]*?<\/\1\s*>|<>[^]*?<\/>|(?<![$\w])<[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*(?:\s+[$\w{}.:-]+(?:\s*=\s*(?:\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}|"[^"]*"?|'[^']*'?|[^"'>\s][^>\s]*))?)*\s*\/?>|<\/(?:[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*)?\s*>/,
			flags: "",
			type: "",
			sub: "",
			subRules: [
				{
					re: /<[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*(?:\s+[$\w{}.:-]+(?:\s*=\s*(?:\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}|"[^"]*"?|'[^']*'?|[^"'>\s][^>\s]*))?)*\s*\/?>|<\/(?:[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd\-.0-9\u00b7\u0300-\u036f\u203f-\u2040]*)?\s*>|<>/,
					flags: "",
					type: "",
					sub: "",
					subRules: [
						...
						},
						{
							...
							type: "bracket",
							sub: "",
							subRules: [{ re: /(?<=^\{)[^]+(?=\}$)/, flags: "", type: "", sub: "jsx", subRules: NO_SUB_RULES }],
						},
						{
							re: /=\s*(?:"[^"]*"?|'[^']*'?|[^"'>\s][^>\s]*)/,
							flags: "",
...
const LANG_LESS: ShjLanguageDefinition = {
	rules: [
		...
		{
			re: /@(media|import|charset|namespace|supports|keyframes|font-face|page|plugin|layer|container|property|counter-style|-[\w-]+)(?![\w-]|\s*:)|![\w-]+|(?<![\w-])(and|not|only|or|when|reference|inline|once|multiple|optional)(?![\w-])/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /(\.\d+|\d+(\.\d+)?)(cm|mm|in|px|pt|pc|em|ex|ch|rem|vmin|vmax|vw|vh|deg|rad|turn|ms|s|fr|%)?/,
			flags: "",
			type: "num",
			sub: "",
			subRules: [{ re: /[a-z]+|%/, flags: "", type: "var", sub: "", subRules: NO_SUB_RULES }],
...
const LANG_LOG: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /(\.e?|\b)\d(e-|[\d.oxa-fA-F_])*(\.|\b)/, flags: "", type: "num", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b(err(or)?|[a-z_-]*exception|warn|warning|failed|ko|invalid|not ?found|alert|fatal)\b/i,
			flags: "i",
			type: "err",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_LUA: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /(["'])(\\[^]|(?!\1)[^\r\n\\])*\1?/, flags: "", type: "str", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b(and|break|do|else|elseif|end|for|function|if|in|local|not|or|repeat|return|then|until|while)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_PHP: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /<<<'(\w+)'[^]*?\n[ \t]*\1\b/, flags: "", type: "str", sub: "", subRules: NO_SUB_RULES },
		{
			...
			subRules: [
				{
					re: /\\(u\{[\da-f]+\}|x[\da-f]{1,2}|[0-7]{1,3}|[^])/i,
					flags: "i",
					type: "esc",
					sub: "",
					subRules: NO_SUB_RULES,
		...
		{ re: /#\[/, flags: "", type: "kwd", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b0[box][\da-f_]+|(\b\d[\d_]*\.?[\d_]*|\B\.\d[\d_]*)(e[+-]?\d+)?/i,
			flags: "i",
			type: "num",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /(?<=\(\s*)(array|binary|bool|boolean|double|float|int|integer|object|real|string|unset)(?=\s*\))/i,
			flags: "i",
			type: "type",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /\b(bool|callable|float|int|iterable|mixed|never|object|string|void)\b/,
			flags: "",
			type: "type",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /(?<=\b(?:class|enum|extends|implements|instanceof|interface|new|trait)\s+|#\[\s*)\\?\w+(\\\w+)*/,
			flags: "",
			type: "class",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_PL: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /(\.e?|\b)\d(e-|[\d.oxa-fA-F_])*(\.|\b)/, flags: "", type: "num", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b(any|break|continue|default|delete|die|do|else|elsif|eval|for|foreach|given|goto|if|last|local|my|next|our|package|print|redo|require|return|say|state|sub|switch|undef|unless|until|use|when|while|not|and|or|xor)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
	...
	rules: [
		{
			...
			type: "cmnt",
			sub: "",
			subRules: [{ re: /(?<=^[ \t]*\.)[a-z]+\b/im, flags: "im", type: "kwd", sub: "", subRules: NO_SUB_RULES }],
		},
		{ re: /#.*/, flags: "", type: "", sub: "todo", subRules: NO_SUB_RULES },
		{
			re: /@"[^]*?^"@/m,
		...
		{
			re: /\b(?:begin|break|catch|class|continue|data|default|do|dynamicparam|else|elseif|end|enum|exit|filter|finally|foreach|for|from|function|hidden|if|inlinescript|in|parallel|param|process|return|sequence|static|switch|throw|trap|try|until|using|while|workflow)\b/i,
			flags: "i",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /-(?:eq|ne|ge|gt|le|lt|notlike|like|notmatch|match|notcontains|contains|notin|in|isnot|is|as|and|or|xor|not|band|bor|bxor|bnot|shl|shr|replace|split|join|f)\b/i,
			flags: "i",
			type: "oper",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /\b0x[\da-f]+\b|(?:\.\d|\b\d)\d*(?:\.\d+)?(?:e[+-]?\d+)?(?:kb|mb|gb|tb|pb)?\b/i,
			flags: "i",
			type: "num",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_PY: ShjLanguageDefinition = {
	rules: [
			...
			subRules: [
				{
					...
					type: "var",
					sub: "",
					subRules: [{ re: /(?!^{)[^]*(?=}$)/, flags: "", type: "", sub: "py", subRules: NO_SUB_RULES }],
				},
			],
		},
		...
		{ re: /(["'])(\\[^]|(?!\1)[^\r\n\\])*\1?/, flags: "", type: "str", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b(and|as|assert|async|await|break|class|continue|def|del|elif|else|except|finally|for|from|global|if|import|in|is|lambda|nonlocal|not|or|pass|raise|return|try|while|with|yield)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_RB: ShjLanguageDefinition = {
	rules: [
		...
		{
			re: /%[qQwWiIrsx]?(?:\[(?:\\[^]|\[[^[\]]*\]|[^[\]])*\]|\((?:\\[^]|\([^()]*\)|[^()])*\)|\{(?:\\[^]|\{[^{}]*\}|[^{}])*\}|<(?:\\[^]|<[^<>]*>|[^<>])*>)[a-z]*|%[qQwWiIrsx]([^\s\w])(?:\\[^]|(?!\1)[^])*\1/,
			flags: "",
			type: "str",
			sub: "",
			subRules: [
		...
		{
			re: /\b0[box][\da-f_]+r?i?|\b\d[\d_]*(?:\.\d[\d_]*)?(?:e[+-]?\d+)?r?i?\b/i,
			flags: "i",
			type: "num",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_RS: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /(\.e?|\b)\d(e-|[\d.oxa-fA-F_])*(\.|\b)/, flags: "", type: "num", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b(as|break|const|continue|crate|else|enum|extern|false|fn|for|if|impl|in|let|loop|match|mod|move|mut|pub|ref|return|self|Self|static|struct|super|trait|true|type|unsafe|use|where|while|async|await|dyn|abstract|become|box|do|final|macro|override|priv|typeof|unsized|virtual|yield|try)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_SCSS: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /\$[\w-]+/, flags: "", type: "var", sub: "", subRules: NO_SUB_RULES },
		{
			re: /@[\w-]+|![\w-]+|\b(and|as|else|from|hide|if|in|not|only|or|show|through|to|using|with)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_SQL: ShjLanguageDefinition = {
	rules: [
		...
		{ re: /(["'])(\\[^]|(?!\1)[^\r\n\\])*\1?/, flags: "", type: "str", sub: "", subRules: NO_SUB_RULES },
		{
			re: /\b(AVG|COUNT|FIRST|FORMAT|LAST|LCASE|LEN|MAX|MID|MIN|MOD|NOW|ROUND|SUM|UCASE)(?=\s*\()/i,
			flags: "i",
			type: "func",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /[-+*/=%^~]|&&?|\|\|?|!=?|<(?:=>?|<|>)?|>[>=]?|\b(?:AND|BETWEEN|DIV|IN|ILIKE|IS|LIKE|NOT|OR|REGEXP|RLIKE|SOUNDS LIKE|XOR)\b/i,
			flags: "i",
			type: "oper",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_SWIFT: ShjLanguageDefinition = {
	rules: [
			...
			subRules: [
				{
					...
					type: "esc",
					sub: "",
					subRules: [{ re: /(?<=^\\#+\()[^]*(?=\)$)/, flags: "", type: "", sub: "swift", subRules: NO_SUB_RULES }],
				},
			],
		},
		{
		...
		{
			re: /\b0[xX][\da-fA-F_]+(?:\.[\da-fA-F_]+)?(?:[pP][+-]?\d+)?|\b0[bBoO][\d_]+|\b\d[\d_]*(?:\.\d[\d_]*)?(?:[eE][+-]?\d+)?/,
			flags: "",
			type: "num",
			sub: "",
			subRules: NO_SUB_RULES,
...
const LANG_TS: ShjLanguageDefinition = {
	rules: [
		{
			re: /:\s*(any|void|number|boolean|string|object|never|enum)\b/,
			flags: "",
			type: "type",
			sub: "",
			subRules: NO_SUB_RULES,
		...
		{
			re: /\b(type|namespace|typedef|interface|public|private|protected|implements|declare|abstract|readonly)\b/,
			flags: "",
			type: "kwd",
			sub: "",
			subRules: NO_SUB_RULES,
			...
			subRules: [
				{
					...
					subRules: [
						...
						},
						{
							...
							type: "bracket",
							sub: "",
							subRules: [{ re: /(?<=^\{)[^]+(?=\}$)/, flags: "", type: "", sub: "tsx", subRules: NO_SUB_RULES }],
						},
						{
							re: /=\s*(?:"[^"]*"?|'[^']*'?|[^"'>\s][^>\s]*)/,
							flags: "",
