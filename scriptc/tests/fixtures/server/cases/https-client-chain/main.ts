/* The served-chain shapes (portless serves leaf+CA concatenated, its
 * SNI contexts include the CA): an untrusting client refuses with
 * Node's "self-signed certificate in certificate chain"; trusting the
 * right CA connects. */
  ...
  const req = request(
    ...
    (res) => {
      res.on("end", () => {
        ...
        tryGet("/text", "default", () => {
          tryGet("/text", "right-ca", () => {
            tryGet("/quit", "right-ca", () => console.log("done"));
          });
        });
      });
      res.on("data", () => {});
